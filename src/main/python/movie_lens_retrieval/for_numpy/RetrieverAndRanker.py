...
class RetrieverAndRanker:
  ...
  def __init__(self, user_movie_saved_model_dir:str, metadata_saved_model_dir:str,
    ...
    self.shift_bits = 13
    self.user_bloom_filter, self.user_movie_bloom_filter = RetrieverAndRanker._init_rbloom(ratings_pl, self.shift_bits)
  ...
  def _init_rbloom(ratings: pl.DataFrame, shift_bits:int) -> Tuple[Bloom, Bloom]:
    """
    ...
    for row_tuple in ratings.iter_rows():
      um_bf.add(row_tuple[user_idx] << shift_bits + row_tuple[movie_idx])
    return u_bf, um_bf
  ...
  def get_users_given_users(self, user_data_dict:Dict[str, Union[int, bytes]], top_k:int):
    ...
    if RetrieverAndRanker.is_linux:
      ...
      neighbor_idxs, distances = self.user_indexers.search_batched(user_embeddings, top_k)
    else:
