...
class Retriever:
    ...
    def __init__(self, user_movie_saved_model_dir: str,
            ...
            cold_start_movie_list: List[int], max_k: int = 1000,
        ...
        self.cold_start_movie_list = cold_start_movie_list.copy()
        
        self.loaded_user_movie_model = tf.saved_model.load(
            user_movie_saved_model_dir)
        ...
        self.user_indexers, self.users_ht, self.user_age_ht = self._create_user_indexer(
            users_path,
            self.loaded_user_movie_model, self.max_k,
            batch_size=users_batch_size)
        self.movie_indexers, self.movies_ht, self.movie_genres_ht = self._create_movie_indexer(
            movies_path,
            self.loaded_user_movie_model, self.max_k,
            batch_size=movies_batch_size)
    
    def _create_movie_indexer(self, movies_path: str,
            loaded_user_movie_model, max_k: int, batch_size: int = 256) \
            -> Tuple[
                scann.scann_ops_pybind.ScannSearcher, tf.lookup.StaticHashTable, tf.lookup.StaticHashTable]:
        """
        ...
        candidate_model = loaded_user_movie_model.signatures[
            "serving_candidate"]
        INPUT_KEY = list(candidate_model.structured_input_signature[1].keys())[
            0]
        embeddings = []
    ...
    def _create_user_static_hashtables(self, ds_ser: tf.data.Dataset,
            batch_size: int = 256) \
    ...
    def _create_movie_static_hashtables(self, ds_ser: tf.data.Dataset,
            batch_size: int = 256) \
            -> Tuple[tf.lookup.StaticHashTable, tf.lookup.StaticHashTable]:
        ...
        def clean_genres(features):
            features['genres'] = tf.strings.regex_replace(
                input=features['genres'],
                pattern="Children's", rewrite="Children")
            return features
        
        ds = ds_ser.map(lambda x: parse_tf_example(x, feature_spec)).batch(
            batch_size)
    ...
    def _create_dictionary_of_tensors(self,
            inputs: List[Dict[str, Union[int, str]]]):
        """
        ...
        for inp_dict in inputs:
            ...
            if isinstance(m_id, list) and not isinstance(genres, list) \
                ...
                raise ValueError(
                    "genres and m_id must both be lists or scalars")
                ...
                for key in final_keys:
                    if key not in reqs:
                        if key in inp_dict:
                            ...
                            outp_dict[key].append(
                                [975768870])  # latest time in training dataset
                        elif key == "gender":
    ...
    def _create_serialized_tfexample(
            inputs: Dict[str, Union[int, str]]) -> bytes:
        expected_keys = {'user_id': int, 'movie_id': int, 'rating': int,
            "timestamp": int,
        ...
        try:
            ...
            logging.error(
                f"ERROR: {ex}, name={name}, value={value}, element_type={element_type}")
            raise ex
        try:
            ...
            logging.error(
                f"ERROR: {ex}, out_name={out_name}, out_type={out_type}")
            raise ex
    ...
    def _create_user_embeddings(inputs: Union[
        Dict[str, Union[int, str]], List[Dict[str, Union[int, str]]]],
            loaded_user_movie_model,
            user_age_hashtable: tf.lookup.StaticHashTable = None) -> tf.Tensor:
        """
    ...
    def _create_movie_embeddings(inputs: Union[
        ...
        for inp_dict in inputs:
            ...
            if movie_genres_hashtable and "genres" not in inp_dict:
                genres = movie_genres_hashtable.lookup(
                    tf.constant(movie_id, dtype=tf.int64))
                if genres == b"":
                    raise ValueError(
                        f"movie_id {movie_id} is not registered, so 'genres' is missing from inputs")
    ...
    def _create_user_indexer(self, users_path: str, loaded_user_movie_model,
            max_k: int, batch_size: int = 256) -> Tuple[
        scann.scann_ops_pybind.ScannSearcher, tf.lookup.StaticHashTable, tf.lookup.StaticHashTable]:
    ...
    def get_users_given_users(self, user_data_dict: Union[
        ...
        if top_k > self.max_k:
            top_k = self.max_k
        embeddings_tensor = Retriever._create_user_embeddings(user_data_dict,
            self.loaded_user_movie_model,
            self.user_age_ht)
        neighbor_idxs, distances = self.user_indexers.search_batched(
            embeddings_tensor, top_k)
        nearest_user_ids = [[int(idx) for idx in
            self.users_ht.lookup(tf.constant(_list, dtype=tf.int64)).numpy()]
            for _list in neighbor_idxs]
    ...
    def get_movies_given_movies(self, movie_data_dict: Union[
        Dict[str, Union[int, str]], List[Dict[str, Union[int, str]]]],
            top_k: int):
        ...
        movie_embeddings = Retriever._create_movie_embeddings(movie_data_dict,
            self.loaded_user_movie_model,
            self.movie_genres_ht)
        neighbor_idxs, distances = self.movie_indexers.search_batched(
            movie_embeddings, top_k)
        nearest_movie_ids = [[int(idx) for idx in
            self.movies_ht.lookup(tf.constant(_list, dtype=tf.int64)).numpy()]
            for _list in neighbor_idxs]
    ...
    def get_users_given_movies(self, movie_data_dict: Union[
        Dict[str, Union[int, str]], List[Dict[str, Union[int, str]]]],
            top_k: int):
        if top_k < 1:
            raise ValueError('top_k must be >= 1')
    ...
    def get_movies_given_users(self, user_data_dict: Union[
        Dict[str, Union[int, str]], List[Dict[str, Union[int, str]]]],
            top_k: int):
        if top_k < 1:
            raise ValueError('top_k must be >= 1')
        if top_k > self.max_k:
            top_k = self.max_k
        user_embeddings = Retriever._create_user_embeddings(user_data_dict,
            self.loaded_user_movie_model,
            self.user_age_ht)
        neighbor_idxs, distances = self.movie_indexers.search_batched(
            user_embeddings, top_k)
        nearest_movie_ids = [[int(idx) for idx in
            self.movies_ht.lookup(tf.constant(_list, dtype=tf.int64)).numpy()]
            for _list in neighbor_idxs]
        return nearest_movie_ids
    ...
    def user_is_known(self, user_id) -> bool:
        lookup_id = self.users_ht.lookup(tf.constant(user_id, dtype=tf.int64))
        return False if lookup_id == -1 else True
